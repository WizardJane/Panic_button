...
byte* audio[FLASH_RECORD_SIZE + headerSize];

void send_geoposition_and_audio() {
    client.publish("/miptfab/esp32led/geoposition/", geoposition.c_str());
    ...
    client.publish("/miptfab/esp32led/audio/", (uint8_t *) audio, (unsigned int) (FLASH_RECORD_SIZE + headerSize));
    // sendData("/miptfab/esp32led/audio/", (uint16_t*) audio);
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqttUsr, mqttPass)) {
      ...
      client.publish("/miptfab/esp32led/audio/", "connected");
      client.publish("/miptfab/esp32led/geoposition/", "connected");
...
void SPIFFSInit(){
  ...
  //printf("header:");
  //example_disp_buf((uint8_t*) header, headerSize);
...
void i2s_adc()
{
    ...
    uint8_t* flash_write_buff = (byte*) calloc(i2s_read_len, sizeof(char));
    ...
    while (flash_wr_size < FLASH_RECORD_SIZE) {
        ...
        for (int i = 0; i < i2s_read_len; i++) {
          audio[index] = (uint8_t*) flash_write_buff[i];
          index++;
        }
...
void loop() {
  ...
  // Serial.print("recording_available ");
  // Serial.println(recording_available);
  ...
  if (recording_available) {
    ...
    if (buttonState == HIGH && recording_available) {
      delay(1500);
      if (buttonState == HIGH && recording_available) {
        ...
        i2s_adc();
        //xTaskCreate(i2s_adc, "i2s_adc", 1024 * 6, NULL, 0, NULL);
